...
- **DSH 0.1.7 generic code banner**: when the host omits fenced-code language metadata and shows a generic CodeBlock, the DOM channel recognizes only complete JSON documents that pass the existing GenUI schema validation. An explicit language label always takes priority; other languages and invalid documents remain ordinary code blocks.
