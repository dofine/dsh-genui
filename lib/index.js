...
function nodeSlotAt(root, path) {
	...
	if (steps.length === 0 || steps[steps.length - 1][1] !== "items") return void 0;
	let current = root;
	for (let i = 0; i < steps.length - 1; i++) {
		const step = steps[i];
		const holder = obj(current);
		const list = holder === void 0 ? void 0 : holder[step[1]];
		current = Array.isArray(list) ? list[Number(step[2])] : void 0;
		if (current === void 0) return void 0;
	}
	const holder = obj(current);
	const list = holder === void 0 ? void 0 : holder.items;
	if (!Array.isArray(list)) return void 0;
	return {
		array: list,
		index: Number(steps[steps.length - 1][2])
	};
}
...
function partialRepairGenuiSpec(processed) {
	...
	if (root === void 0 || isComponentRoot(root)) return null;
